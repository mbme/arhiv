...
run:
  cd arhiv; npm run clean; tmux new-session -s arhiv \
     'watchexec -r --debounce=4000 --exts rs -- "notify-send Restarting... -t 2000; DEBUG_ARHIV_ROOT=~/temp/arhiv  RUST_BACKTRACE=1 RUST_LOG=debug,mdns_sd=info,rs_utils=info,hyper=info cargo run -p binutils --bin arhiv server"' \; \
