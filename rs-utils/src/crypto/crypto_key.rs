...
impl CryptoKey {
    ...
    pub fn derive_from_password_with_argon2(password: &SecretBytes, salt: &str) -> Result<Self> {
        ensure!(
            ...
            "salt must consist of at least {} characters",
    ...
    pub fn from_crypto_bytes(key: impl Into<SecretBytes>) -> Result<Self> {
        let key = key.into();
...
mod tests {
    ...
    fn test_derive_subkey() -> Result<()> {
        {
            let key1 =
                CryptoKey::from_crypto_bytes([1; 32].as_slice())?.derive_subkey("salt1234")?;
            let key2 =
                CryptoKey::from_crypto_bytes([1; 32].as_slice())?.derive_subkey("salt1234")?;
...

        {
            let key1 =
                CryptoKey::from_crypto_bytes([1; 32].as_slice())?.derive_subkey("salt1234")?;
            let key2 =
                CryptoKey::from_crypto_bytes([1; 32].as_slice())?.derive_subkey("salt12345")?;
