...
arhiv *PARAMS:
  ...
  DEV_ARHIV_ROOT="{{root}}" SERVER_PORT=8443 cargo run -p arhiv-cli --bin arhiv {{PARAMS}}

run:
  cd arhiv; npm run clean; tmux new-session -s arhiv \
     'DEV_ARHIV_ROOT={{root}} SERVER_PORT=8443 RUST_LOG={{debug_log_level}} BROWSER=chromium cargo run -p arhiv-cli --bin arhiv server --browser' \; \
