...
export const scrapeFilmFromIMDB: Scraper<IMDBFilm> = async (locationURL) => {
  if (locationURL.hostname !== 'www.imdb.com' || getPathSegments(locationURL)[0] !== 'title') {
    return undefined;
  }
  ...
  if (isSeries) {
    seasons = Number.parseInt(
      getEl('[data-testid=episodes-browse-episodes] >:nth-child(2) >:nth-child(2)', 'seasons count')
        .innerText,
      10
    );
