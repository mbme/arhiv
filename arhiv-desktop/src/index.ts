...
async function handleAction(action: Action, baseUrl: string) {
  ...
  switch (action.type) {
    case 'search': {
      myWindow.webContents.send('search', action.query);
      break;
    }
    case 'new': {
      myWindow.webContents.send('new');
      break;
    }
    case 'open': {
      if (action.documentId) {
        myWindow.webContents.send('open', action.documentId);
      }
      break;
    }
  }
...
async function start(args: string[]) {
  ...
  win.loadURL(serverInfo.url).catch(() => {
    console.error('failed to open Arhiv');
  });
